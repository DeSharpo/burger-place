...
const mockOrders = [
	...
	{ _id: '2', number: 333, status: 'pending', ingredients: ['3'] },
	{ _id: '3', number: 222, status: 'done', ingredients: ['1'] },
];
...
describe('profile-orders reducer', () => {
	...
	it('wsGetOrders should set sorted orders and totals', () => {
		const payload = {
			orders: mockOrders,
			total: 200,
			totalToday: 120,
		...
		expect(state.orders.map((o) => o.number)).toEqual([333, 222, 111]);
		expect(state.total).toBe(200);
		expect(state.totalToday).toBe(120);
	});
});
